...
public unsafe sealed class UserInterface : NeoUIEntry
{
    ...
    public override NuiBuilder Builder => new NuiBuilder()
        ...
        .Widget(150f, "Plugin display language".Loc(), (x) =>
        {
            ...
            {
                foreach(var lang in Localization.GetAvaliableLanguages())
                {
                    if(ImGui.Selectable(LanguageDisplayNames.GetValueOrDefault(lang, lang), lang == current) && lang != current)
                    {
                        C.PluginLanguage = lang;
                        Localization.Init(lang == "English" ? null : lang);
                        S.NeoWindow.Reload();
                    }
                }
                ImGui.EndCombo();
        ...
        .Checkbox("Skip item sell/trade confirmation while plugin is active".Loc(), () => ref C.SkipItemConfirmations)
